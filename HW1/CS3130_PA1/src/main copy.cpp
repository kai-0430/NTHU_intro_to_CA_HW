...
*/
    ...
    //bool has_pushed_to_prime_impli = 0;
    for (int i = 0; i < nSize - 1; i++) {
        if (!impli_by_size[i].empty()) {
            //has_pushed_to_prime_impli = 0;
            ...
            for (int j = i + 1; j < nSize - 1; j++) {
                ...
                if (!impli_by_size[j].empty()) {
                    ...
                    merge_implicnats_in_one_set(impli_by_size, i + 1);  // merge for next iteration
                    //has_pushed_to_prime_impli = 1;
                }
            }
