...
void FM_algorithm::initial_partition()
{
    ...
    if (k == 2)
    {
        ...
        for (i = 0; partition_size[0] > max_area && i < half; i++)
        {
        ...
        for (i = half; partition_size[1] > max_area && i < n_cells; i++)
        {
            if (partition_size[0] + cell[i].size < max_area)
            {
                cell[i].group = 0;
                partition_size[1] -= cell[i].size;
                partition_size[0] += cell[i].size;
...
int FM_algorithm::choose_base_cell()
{
    ...
    while (move_cell == -1)
    {
        ...
        /* determine by balancement */
        vector<int> diff(candidate.size());    // difference between the new_size and average_size
        for (unsigned int i = 0; i < candidate.size(); i++)
        {
            ...
            if (new_size > max_area)
                ...
                diff[i] = std::abs(new_size - total_size/2);
        }

        int min = INT32_MAX;
        for (unsigned int i = 0; i < candidate.size(); i++)
        {
            if (diff[i] != -1 && diff[i] < min)
            {
    ...
    else
    {
        ...
        int T = !F;                     // to cell
        int balancy = std::abs(partition_size[T] + cell[move_cell].size - total_size / 2);
...
//cout << "_0_times: " << _0_times << endl;
    ...
    partition_size = vector<int>(k, 0);     // 2-way, 2 partitions
    for (int i = 0; i < n_nets; i++)
        net_distri[i] = vector<int>(k, 0);
    compute_initial_info();
